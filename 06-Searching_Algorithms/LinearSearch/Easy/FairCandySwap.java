...
public class FairCandySwap {
    ...
    public static int[] fairCandySwap(int[] aliceSizes, int[] bobSizes) {
        int aliceSum = 0;
        int bobSum = 0;
        //we have to calculate the sum of alice array
        for (int a = 0; a < aliceSizes.length; a++) {
            aliceSum = aliceSum + aliceSizes[a];
        }
        //then we have to calculate the sum of bob array
        for (int b = 0; b < bobSizes.length; b++) {
            bobSum = bobSum + bobSizes[b];
        }
        //then we have to find the differerence to balance the total amount of candy for both alice & bob
        //diff = (sum1 - sum2)/2
